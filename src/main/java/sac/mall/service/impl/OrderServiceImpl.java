...
import sac.mall.common.*;
import sac.mall.controller.vo.*;
import sac.mall.dao.GoodsMapper;
import sac.mall.dao.OrderItemMapper;
import sac.mall.dao.OrderMapper;
import sac.mall.dao.ShoppingCartItemMapper;
import sac.mall.entity.Goods;
import sac.mall.entity.Order;
import sac.mall.entity.OrderItem;
import sac.mall.entity.StockNumDTO;
import sac.mall.service.OrderService;
import sac.mall.util.BeanUtil;
import sac.mall.util.NumberUtil;
...
public class OrderServiceImpl implements OrderService {
    ...
    public String saveOrder(UserVO user, List<ShoppingCartItemVO> myShoppingCartItems) {
        ...
        for (ShoppingCartItemVO shoppingCartItemVO : myShoppingCartItems) {
            ...
            if (shoppingCartItemVO.getGoodsCount() > newBeeMallGoodsMap.get(shoppingCartItemVO.getGoodsId()).getStockNum()) {
                NewShopMallException.fail(ServiceResultEnum.SHOPPING_ITEM_COUNT_ERROR.getResult());
            }
        }
        //删除购物项
        if (!CollectionUtils.isEmpty(itemIdList) && !CollectionUtils.isEmpty(goodsIds) && !CollectionUtils.isEmpty(goods)) {
            if (shoppingCartItemMapper.deleteBatch(itemIdList) > 0) {
                ...
                if (priceTotal < 1) {
                    NewShopMallException.fail(ServiceResultEnum.ORDER_PRICE_ERROR.getResult());
                }
                ...
                if (orderMapper.insertSelective(order) > 0) {
                    ...
                    //保存至数据库
                    if (newBeeMallOrderItemMapper.insertBatch(orderItems) > 0) {
                        //所有操作成功后，将订单号返回，以供Controller方法跳转到订单详情
                        return orderNo;
                    }
                    NewShopMallException.fail(ServiceResultEnum.ORDER_PRICE_ERROR.getResult());
                }
                NewShopMallException.fail(ServiceResultEnum.DB_ERROR.getResult());
            }
            NewShopMallException.fail(ServiceResultEnum.DB_ERROR.getResult());
        }
        NewShopMallException.fail(ServiceResultEnum.SHOPPING_ITEM_ERROR.getResult());
    ...
    public OrderDetailVO getOrderDetailByOrderNo(String orderNo, Long userId) {
        ...
        if (order != null) {
            ...
            List<OrderItem> orderItems = newBeeMallOrderItemMapper.selectByOrderId(order.getOrderId());
            //获取订单项数据
            if (!CollectionUtils.isEmpty(orderItems)) {
                ...
                newBeeMallOrderDetailVO.setNewBeeMallOrderItemVOS(orderItemVOS);
